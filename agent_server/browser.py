...
Replaces several overlapping implementations that did not share state: five
`browser-*` tools sharing one global page, and a `screenshot` tool that launched
a fresh browser for every call and closed it again. So logging in with
...
* **Every frame is written to disk and its path returned.** The `browser-*`
  tools threw the bytes away, so a before/after comparison was impossible and
  nothing could be re-examined without redoing the whole flow. This tool drives
  and asserts; what happens to a saved frame afterwards is not its concern.
