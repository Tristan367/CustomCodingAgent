...
def _blocks(text: str) -> list[str]:
    ...
    A newline in markdown source is usually just where the author's editor
    wrapped, not a pause. Treating every one as a boundary chopped sentences in
    half mid-clause. Only a blank line, a heading or a list marker really starts
    something new; everything else is joined back onto the line above.
    """
    out: list[str] = []
    fresh = True
...
async def synth(text: str, voice: str = "", speed: float = 1.0) -> bytes:
    ...
    return encode_wav(pad_edges(audio, rate), rate)
