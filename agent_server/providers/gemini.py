...
Google publishes an OpenAI-compatible endpoint alongside its native one, so this
is the base class with a different URL rather than a second wire format. See
https://ai.google.dev/gemini-api/docs/openai .

The Flash models have a real free tier, which makes Gemini the cheap seat for
the work that does not need a frontier model -- a subagent grepping a tree, a
compaction summariser, a long autonomous run where the per-token cost is what
decides how long you can let it go.
"""
